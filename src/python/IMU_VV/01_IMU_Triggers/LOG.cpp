...
// Github: https://github.com/VADL-2022/VADL-2021-Source-LEGACY/blob/master/src/LOG.cpp
...
//LOG::LOG(IMU* imu, LIDAR* lidar, LDS* lds) : mImu(imu), mLidar(lidar), mLds(lds)
LOG::LOG(IMU* imu) : mImu(imu)
{
    if (LOG_ACTIVE || VERBOSE)
    {
        ...
        // Change this from timestamp to somethign human-readable
        // At least it is confirmed that it's a timestamp now
